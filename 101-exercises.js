...
function areaOfRectangle(x1, x2) {
    return (x1 * x2)
}
...
function hasVowels(string) {
    ...
    for (var x = 0; x < string.length; x++) {
        if (vowels.toLowerCase().indexOf(string[x]) !== -1) {
            vcount++;
        }
    }
    return vcount > 0;
...
function startsAndEndsWithVowel(string) {
    if (!startsWithVowel(string[0]) && !endsWithVowel(string[0])) {
        return false;
    } else {
        return true;
    }
...
function totalNumberOfItems(shoppingCart) {
        ...
        allAmount += shoppingCart.items[i].quantity;
...
function getAverageItemPrice(shoppingCart) {
    var avgItem = 0;
    for (var i = 0; i < shoppingCart.items.length; i++) {
        avgItem += shoppingCart.items[i].price;

    }
    return avgItem/shoppingCart.items.length
